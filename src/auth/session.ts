...
export async function createOwnerSession(
  ...
  const session: OwnerSession = { ...input, exp: Math.floor(now.getTime() / 1000) + ttlSeconds };
...
export async function readOwnerSession(value: string | undefined, secret: string, now = new Date()): Promise<OwnerSession | null> {
  ...
  try {
    ...
    return { sub: parsed.sub, email: parsed.email, name: parsed.name, exp: parsed.exp };
  } catch {
    return null;
  }
}
...
export function ownerAuthenticator(config: {
  sessionSecret: string;
  ownerApiKey?: string;
  apiOwnerId?: string;
}): OwnerAuthenticator {
  return async (request) => {
    const session = await readOwnerSession(cookie(request, SESSION_COOKIE), config.sessionSecret);
    if (session) return `google:${session.sub}`;

    const authorization = request.headers.get("authorization");
    if (config.ownerApiKey && authorization?.startsWith("Bearer ")) {
      const supplied = authorization.slice(7).trim();
      if (supplied && safeEqual(supplied, config.ownerApiKey)) return config.apiOwnerId || "owner-agent";
    }
    return null;
  };
}
